...
class DistriTransformer2DModel(BaseModule):
    ...
    def forward(
        ...
        if (
            ...
            logger.info(f"rank {distri_config.rank} ori hidden_states.shape: {hidden_states.shape}")
            b, c, h = hidden_states.shape
            sliced_hidden_states_group = hidden_states.split((c + micro_batch_size - 1) // micro_batch_size, dim=1)
            async_handle = []
