...
FP4 and INT8 are post-load conversions during ordinary loading. FP8 uses native quantize-on-load when the selected Diffusers/Transformers APIs are available: AITER for diffusers transformers and Transformers 5 text encoders, and torchao for diffusers transformers and ordinary pipeline-loaded text encoders. Safe native routing is derived from each component's config-built structure; if exact xDiT target prefixes cannot be represented, the component is logged and converted post-load instead of broadening coverage. Memory-efficient FSDP and replicated meta-load retain their blockwise/broadcast placement. Replicated torchao text encoders use post-load conversion; memory-efficient FSDP rejects a text-encoder fallback before allocation because its meta-fill path cannot safely change layouts after sharding.

#### Model Support Matrix

“Streaming” below means eligible for native transformer quantization during ordinary loading: AITER on RDNA4, or torchao through supported Diffusers APIs on other FP8 hardware. Torchao falls back explicitly to post-load conversion when its API, exact target mapping, or placement cannot stream. “Memory-efficient FSDP” means `--memory_efficient_sharding` can avoid materializing the full transformer before FSDP wraps it; “replicated meta-load” means `--memory_efficient_replicated_load` can build the transformer on meta and fill it from rank 0.
...
- Native torchao text-encoder loading requires `torchao>=0.15.0`, Diffusers `PipelineQuantizationConfig`, and Transformers `TorchAoConfig` quantize-on-load APIs. Native torchao transformer loading separately requires Diffusers `TorchAoConfig`. These APIs are feature-probed lazily; older compatible Diffusers/Transformers installations fall back explicitly to post-load torchao FP8 where placement permits it.
