...
from lagecy.pipefuser.utils import DistriConfig, PipelineParallelismCommManager
            ...
            for i, t in enumerate(warmup_timesteps):
                ...
                if distri_config.rank == 1 and i == 0:
                    ...
                    if distri_config.rank == 0 or distri_config.rank > distri_config.n_device_per_batch//2:
                        self.comm_manager.recv_from_skip(latents.dtype, idx=None, is_extra=True)
                ...
                if distri_config.rank >=1 and distri_config.rank <= distri_config.n_device_per_batch//2:
            ...
            if distri_config.rank == 0 or distri_config.rank > distri_config.n_device_per_batch//2:
                for _ in range(len(pip_timesteps)):
                    for batch_idx in range(pp_num_patch):
                        self.comm_manager.recv_from_skip(latents.dtype, idx=batch_idx, is_extra=True)
            ...
            for i, t in enumerate(pip_timesteps):
                for batch_idx in range(pp_num_patch):
                    ...
                    if distri_config.rank == 1 and i == 0:
                        ...
                        if distri_config.rank == 0 or distri_config.rank > distri_config.n_device_per_batch//2:
                            skips = self.comm_manager.get_skip_data(idx=batch_idx, is_extra=True)
