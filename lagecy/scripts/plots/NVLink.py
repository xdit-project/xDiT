...
with plt.style.context(['science', 'ieee']):
    ...
    for i, t in enumerate(memory_sizes):
        for j, val in enumerate(t):
            axes[j].bar(bar_width * (i - .5), val, width=bar_width - 0.02, label=legend[i], color=colors[i], edgecolor='black')
            ...
            if val == 0:
                continue
            axes[j].text(bar_width * (i - .5), val, str(val), ha='center', va='bottom', fontsize=15, )
    ...
    plt.legend(loc="upper left", frameon=False, ncols=7,  bbox_to_anchor=(-2.1, 1.4, 0, 0), fontsize=20)
