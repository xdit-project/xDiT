...
with plt.style.context(['science', 'ieee']):
    ...
    lenged = ['Original', 'Tensor Parallel', 'PipeFusion', 'DistriFusion', 'Seq Parallel (Ulysses)']
    ...
    for i, t in enumerate(memory_sizes):
        for j, test in enumerate(t):
            if test == 0:
                continue
            plt.text(index[j] + bar_width * (i - 1), test, str(test), ha='center', va='bottom', fontsize=8.5)
    ...
    plt.text(index[3] + bar_width * 2.5, 10, 'OOM', ha='center', va='baseline', color="red", fontsize=16)
