        ...
        if do_classifier_free_guidance and split_batch and world_size >= 2:
            ...
            for i in range(2):
                batch_parallel_groups.append(
                    dist.new_group(
                        list(
                            range(i * n_device_per_batch, (i + 1) * n_device_per_batch)
                        )
                    )
                )
            ...
            for i in range(2):
                extra_groups.append(
                    dist.new_group(
                        dist.get_process_group_ranks(batch_parallel_groups[i])
                    )
                )
            ...
            self.global_dp_groups = [dist.new_group([i]) for i in range(world_size)]
            self.local_dp_group = self.global_dp_groups[self.rank]
            self.local_extra_group = dist.new_group(
                dist.get_process_group_ranks(self.local_batch_parallel_group)
            )
...
class PipelineParallelismCommManager:
    def __init__(self, distri_config: DistriConfig):
        ...
        if (
            distri_config.local_batch_parallel_group is not None
            and distri_config.local_dp_group is not None
        ):
                ...
                [
                    dist.new_group(
                        dist.get_process_group_ranks(
                            distri_config.global_batch_parallel_groups[i]
                        )
                    ),
                    dist.new_group(
                        dist.get_process_group_ranks(
        ...
        self.skip_rank = dist.get_global_rank(
            self.batch_parallel_group,
            (batch_world_size - 1 - dist.get_rank(self.batch_parallel_group) + 2)
            % batch_world_size,
        )
    ...
    def send_shape_comm(self, tensor: torch.Tensor, is_extra=False) -> torch.Size:
        ...
        dist.send(
            dim,
            dst=self.next_rank,
            group=self.extra_group if is_extra else self.batch_parallel_group,
        )
        dist.send(
            shape,
            dst=self.next_rank,
    ...
    def first_send_to_skip(self, tensor: torch.Tensor, is_extra=False):
        ...
        if self.skip_shape is None:
            ...
            logger.warning(
                f"Skip buffer is already initialized, skip sending skip shape."
            )
    ...
    def first_recv_from_skip(self, dtype: Optional[torch.dtype] = None, is_extra=False):
        ...
        if self.recv_skip_buffer is None:
            ...
            self.recv_skip_buffer.append(
                torch.zeros(
                    self.skip_shape, dtype=self.dtype, device=distri_config.device
                )
            )
        else:
            logger.warning(
                f"Recv skip buffer is already initialized, skip receiving skip shape."
            )
        ...
        if self.recv_skip_req is None and len(self.recv_skip_queue) > 0:
            ...
            if idx is None:
                self.recv_skip_req = dist.irecv(
                    self.recv_skip_buffer[-1],
                    src=self.skip_rank,
                    group=self.extra_group if is_extra else self.batch_parallel_group,
                )
            else:
                self.recv_skip_req = dist.irecv(
                    self.recv_skip_buffer[idx],
                    src=self.skip_rank,
                    group=self.extra_group if is_extra else self.batch_parallel_group,
                )
...
class PatchParallelismCommManager:
    ...
    def communicate(self):
        ...
        handle = dist.all_gather(
            buffer_list,
            tensor,
            group=self.distri_config.local_batch_parallel_group,
            async_op=True,
        )
