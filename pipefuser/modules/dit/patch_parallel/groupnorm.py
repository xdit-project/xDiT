        ...
        if distri_config.mode in ["stale_gn", "corrected_async_gn"]:
            ...
            if self.buffer_list is None:
                ...
                if distri_config.mode == "corrected_async_gn":
                    correction = (
                        slice_mean - self.buffer_list[distri_config.split_idx()]
                    )
                    full_mean = (
                        sum(self.buffer_list) / distri_config.n_device_per_batch
                        + correction
                    )
                else:
            ...
            if self.counter <= distri_config.warmup_steps or distri_config.mode in [
                "sync_gn",
                "full_sync",
            ]:
                x = x.view([n, num_groups, group_size, h, w])
                ...
                dist.all_reduce(
                    mean, op=dist.ReduceOp.SUM, group=distri_config.local_batch_parallel_group
                )
