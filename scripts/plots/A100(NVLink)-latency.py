...
memory_sizes = [
    ...
    [1.221, 7.066, 36.33, 455.9], # Tensor Parallel
    # [1.091, 7.029, 80.34, 1183], # PipeFusion (4-Step Warm-up)
    [0.9686, 4.061, 45.04, 616.8], # PipeFusion (1-Step Warm-up)
    [0.7653, 2.476, 25.24, 0], # DistriFusion
    [1.37, 2.46, 23.31, 0], # Seq Parallel (Ulysses)
    [2.59, 3.54, 27.41, 0], # Seq Parallel (Ring)
]
...
with plt.style.context(['science', 'ieee']):
    ...
    for i, t in enumerate(memory_sizes):
        for j, val in enumerate(t):
            axes[j].bar(bar_width * (i - 2.5), val, width=bar_width - 0.02, label=legend[i], color=colors[i], edgecolor='black')
            ...
            if val == 0:
                continue
            axes[j].text(bar_width * (i - 2.5), val, str(val), ha='center', va='bottom', fontsize=9, )
    ...
    #            ) 
    axes[-1].bar(bar_width * 0.5, 3000, width=bar_width - 0.02, color=colors[-3], edgecolor='black', alpha=0.1)
    ...
    axes[-1].text(bar_width * 1.5, 500, 'OOM', ha='center', va='baseline', color="red", fontsize=25)
    ...
    plt.legend(loc="upper left", frameon=False, ncols=7,  bbox_to_anchor=(-3.75, 1.2, 0, 0), columnspacing=1, labelspacing=0, fontsize=14)
    ...
    # 显示图形
    # plt.show()
    plt.savefig('latency-A100-NVLink.png', dpi=500, bbox_inches='tight')