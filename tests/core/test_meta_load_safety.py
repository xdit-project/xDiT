...
def test_checkpoint_dtype_cast_preserves_the_models_fp32_modules(runtime):
    """A blanket .to(bf16) would make the disk fill round fp32 weights down."""
...
def test_checkpoint_dtype_cast_leaves_nonpersistent_buffers_and_integers(runtime):
    """Neither is loaded from the checkpoint, so neither adopts its dtype."""
