"""
Accelerator integration tests for xfuser.core.distributed.sharding.

These tests verify the FSDP sharding functionality for transformer models
using a one-rank Gloo control group and accelerator-backed FSDP parameters.

Run with:
    python -m pytest tests/integration/accelerator/core/distributed/test_sharding.py -v
"""
...
def setup_distributed(tmp_path_factory):
    """Create an isolated one-rank control group and always tear it down."""
    started = not dist.is_initialized()
    if started:
        rendezvous = tmp_path_factory.mktemp("sharding") / "gloo-init"
    ...
    try:
        ...
        if started and dist.is_initialized():
...
def test_shard_component_basic(setup_distributed, dit_model):
...
def test_shard_component_with_dtype(setup_distributed, dit_model):
...
def test_shard_component_invalid_attr(setup_distributed, dit_model):
...
def test_shard_component_with_fsdp_kwargs(setup_distributed, dit_model):
...
def test_default_device_id(setup_distributed, dit_model):
...
def test_no_dtype_conversion(setup_distributed, dit_model):
...
def test_parameter_count_preserved(setup_distributed, dit_model):
...
def test_end_to_end_forward_pass(setup_distributed, dit_model):
