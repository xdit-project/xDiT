...
class TestRingFlashAttn(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not {"LOCAL_RANK", "RANK", "WORLD_SIZE"}.issubset(os.environ):
            raise unittest.SkipTest("run with torchrun")
        if int(os.environ["WORLD_SIZE"]) < 2:
            raise unittest.SkipTest("ring attention requires at least two ranks")
        cls.batch_size = 1
        cls.num_heads = 4
        cls.head_dim = 32
        cls.seq_len = 128
    ...
    def _create_test_tensors(self):
        ...
        # Prepare inputs
        q = torch.randn(shape, device=self.device, dtype=self.dtype, requires_grad=False)
        k = torch.randn(shape, device=self.device, dtype=self.dtype, requires_grad=True)
        v = torch.randn(shape, device=self.device, dtype=self.dtype, requires_grad=True)
    ...
    def test_xdit_ring_flash_attn_func(self):
        ...
        output = xdit_ring_flash_attn_func(
            q=local_q, k=local_k, v=local_v, dropout_p=0.0, causal=True, window_size=(-1, -1), group=dist.group.WORLD
        )
        # Compare results
...
# torchrun --nproc_per_node=2 -m unittest tests/integration/accelerator/core/long_ctx_attention/test_ring_flash_attn.py
