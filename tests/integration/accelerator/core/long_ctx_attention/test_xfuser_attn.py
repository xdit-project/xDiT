...
class TestRingFlashAttn(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if int(os.environ["WORLD_SIZE"]) < 2:
            raise unittest.SkipTest("hybrid attention requires at least two ranks")
    ...
    def _create_test_tensors(self, dtype=torch.float16):
        ...
        # Prepare inputs
        q = torch.randn(shape, device=self.device, dtype=dtype, requires_grad=False)
        k = torch.randn(shape, device=self.device, dtype=dtype, requires_grad=False)
        v = torch.randn(shape, device=self.device, dtype=dtype, requires_grad=False)
...
# torchrun --nproc_per_node=4 -m unittest tests/integration/accelerator/core/long_ctx_attention/test_xfuser_attn.py
