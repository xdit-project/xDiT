...
def _require_gfx950_aiter():
    ...
    if "gfx950" not in arch_name:
        pytest.skip(f"Ideogram 4 HD256 FP8 validation requires gfx950, got {arch_name}.")
    ...
    hd256_kernel = aiter_root / "hsa" / "gfx950" / "fmha_v3_fwd" / "fwd_hd256_fp8.co"
    if not hd256_kernel.exists():
        pytest.skip("AITER does not include the gfx950 HD256 FP8 FMHA kernel from PR 3732.")
...
def test_ideogram4_aiter_fp8_attention_hd256(sequence_length):
    ...
    relative_l2 = (
        torch.linalg.vector_norm(output_float - reference_float) / torch.linalg.vector_norm(reference_float)
    ).item()
    ...
    print(f"sequence_length={sequence_length} relative_l2={relative_l2:.6f} cosine_similarity={cosine_similarity:.6f}")
