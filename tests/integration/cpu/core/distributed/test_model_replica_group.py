...
def _model_replica_group_worker(
    ...
    *,
    ...
    try:
        ...
        device_selection = patch.object(parallel_state, "set_device") if mock_device_selection else nullcontext()
        ...
        gathered = replica.all_gather(torch.tensor([rank], dtype=torch.int64, device=device))
...
def _real_gpu_unavailability(torch):
    ...
    if visible_gpus < _WORLD_SIZE:
        reasons.append(f"{_WORLD_SIZE} visible CUDA-compatible GPUs are required; found {visible_gpus}")
...
def _assert_replica_group_results(processes, hung, survivors, results):
    ...
    facts = {
        rank: (dp_rank, replica_ranks, gathered_ranks) for _, rank, dp_rank, replica_ranks, gathered_ranks in results
    }
    assert facts == {
        0: (0, [0, 1], [0, 1]),
        1: (0, [0, 1], [0, 1]),
