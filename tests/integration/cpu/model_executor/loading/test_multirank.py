...
def _run_replicated_te_validation_worker(rank, world_size, init_method, result_queue, *, scenario):
    dist = None
    try:
        import torch
        import torch.distributed as dist
        ...
        if scenario == "persistence_mismatch":
            ...
            if rank == 0:
                left = torch.nn.Parameter(torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32))
                right = (
                    left
                    if scenario == "source_tied"
                    else torch.nn.Parameter(torch.tensor([4.0, 5.0, 6.0], dtype=torch.float32))
                )
            else:
                left = torch.nn.Parameter(torch.empty(2, dtype=torch.float16, device="meta"))
                right = (
                    left
                    if scenario == "peer_tied"
                    else torch.nn.Parameter(torch.empty(2, dtype=torch.float16, device="meta"))
                )
        ...
        try:
            ...
            if scenario == "spec_reconcile":
                ...
                result_queue.put(("returned", rank, "persistence mismatch was accepted"))
    except BaseException:
        result_queue.put(("error", rank, traceback.format_exc()))
    finally:
        if dist is not None and dist.is_initialized():
...
def _mxfp4_fsdp2_worker(rank, world_size, init_method, result_queue):
    ...
    try:
        ...
        mxfp4_linear.aiter = SimpleNamespace(QuantType=quant_type, get_hip_quant=get_hip_quant)
        mxfp4_linear.shuffle_weight = lambda weight, layout: weight

        full_precision_state = {
            name: tensor.detach().cpu().clone()
            for name, tensor in mxfp4_linear.xFuserMXFP4Linear(8, 4, bias=False).state_dict().items()
        }
        layer = mxfp4_linear.xFuserMXFP4Linear(8, 4, bias=False)
        layer._quantize_weights()
        packed_state = {name: tensor.detach().cpu().clone() for name, tensor in layer.state_dict().items()}
...
def test_two_rank_load_failure_raises_on_every_rank_without_hanging(tmp_path, worker, init_name, expected):
...
def test_mxfp4_packed_weight_is_sharded_by_fsdp2(tmp_path):
    ...
    if "mesh" not in inspect.signature(fully_shard).parameters or not hasattr(DeviceMesh, "from_group"):
    ...
    assert all("packed state cannot be loaded after FSDP" in result[5] for result in results)
    assert all(
        "full-precision state cannot replace an FSDP-managed packed parameter" in result[6] for result in results
    )
