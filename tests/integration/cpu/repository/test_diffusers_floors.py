...
def _from_source_marker(repo_root):
    ...
    for node in ast.parse(path.read_text(errors="replace")).body:
        if isinstance(node, ast.Assign) and isinstance(node.value, ast.Constant):
            if any(isinstance(t, ast.Name) and t.id == "DIFFUSERS_FROM_SOURCE" for t in node.targets):
...
def _guarded(tree):
    ...
    for node in ast.walk(tree):
        ...
        swallows = any(
            ...
            or (isinstance(h.type, ast.Tuple) and any(isinstance(e, ast.Name) and e.id in handled for e in h.type.elts))
            for h in node.handlers
        )
        if swallows:
            for stmt in node.body:
                out.update(n for n in ast.walk(stmt) if isinstance(n, (ast.Import, ast.ImportFrom)))
    return out
...
def _imports(node, package, guarded):
    ...
    for sub in ast.walk(node):
        ...
        if isinstance(sub, ast.ImportFrom):
            ...
            if sub.level:
                parts = package.split(".")
                module = ".".join(parts[: len(parts) - sub.level + 1] + ([module] if module else []))
        ...
        for path in sorted(runners.glob("*.py")):
            ...
            for name in classes:
                ...
                while cursor in classes and cursor not in ancestry:
                    ...
                    cursor = next(
                        (b.id for b in classes[cursor].bases if isinstance(b, ast.Name) and b.id in classes),
                        None,
                    )
...
class TestDeclaredFloorsAreReachable(unittest.TestCase):
    def setUp(self):
        ...
        self.assertNotEqual(self.floors, [], "no min_diffusers_version declarations found")

    def _missing(self, reqs):
        return sorted(
            f"{module}:{symbol or '*'}"
            for module, symbol in reqs
            if not (self.tree.has_symbol(module, symbol) if symbol else self.tree.has_module(module))
        )
