...
from xfuser.core.distributed import attention_backend as attention_backend_module
...
def _hadamard_matrix(size: int) -> torch.Tensor:
...
def cpu_hadamard_matrix(monkeypatch):
    matrix = _hadamard_matrix(128).to(torch.bfloat16)
    FP8_HADAMARD_MATRIX.clear()
    FP8_HADAMARD_MATRIX[_HB_DEVICE] = matrix
    monkeypatch.setattr(
        attention_backend_module,
        "_get_fp8_hadamard_matrix",
        lambda head_dim, device: matrix,
        raising=False,
    )
...
class _FakeTransformer(nn.Module):
    def __init__(self, num_layers: int = 3):
        ...
        for i, block in enumerate(self.blocks):
            ...
            block.attn1.register_buffer("fp8_comms_layer_idx", torch.tensor([i], dtype=torch.long))
...
def test_fp8_attention_kwargs_returns_call_when_synced():
    ...
    extras = fp8_attention_kwargs(fp8, attn, q, q, q, False, AttentionBackendType.AITER_FLYDSL_FP8)
...
def test_hybrid_schedule_gates_fp8_comms_by_active_backend():
    ...
    extras_by_step = [
        fp8_attention_kwargs(fp8, attn, q, q, q, False, schedule.get_backend(step))
...
def test_hadamard_matrix_is_orthonormal():
    ...
    torch.testing.assert_close(R @ R.T, torch.eye(R.shape[0], device=R.device), rtol=0, atol=1e-3)
...
def test_calibrated_scale_matches_the_rotated_tensor():
    ...
    fp8.update_running_max(model, torch.tensor([0], dtype=torch.long, device=_HB_DEVICE), q_rot, k_rot, v)
    model_state = fp8.get_model_state(model)

    assert model_state.q_running_max[0].item() == pytest.approx(q_rot.abs().amax().float().item())
    assert model_state.q_running_max[0].item() != pytest.approx(q.abs().amax().float().item())
    ...
    assert (q_rot.float() / q_scale).abs().amax().item() == pytest.approx(fp8_max * safety_factor, rel=1e-2)
