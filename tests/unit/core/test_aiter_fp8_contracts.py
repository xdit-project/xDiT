...
def aiter_fp8_defaults(monkeypatch):
...
def test_aiter_fp8_uses_varlen_metadata(monkeypatch):
    ...
    monkeypatch.setattr(attention_backend, "aiter", fake_aiter, raising=False)
    monkeypatch.setattr(attention_backend, "AITER_FP8_HAS_DESCALE", True, raising=False)
    monkeypatch.setattr(
        attention_backend,
        "FP8_HADAMARD_MATRIX",
    ...
    monkeypatch.setattr(
        ...
        lambda head_dim, device: _feature_reversal_matrix(),
        raising=False,
    )
    ...
    expected_key = key.permute(0, 2, 1, 3).reshape(4, 2, 128).index_select(0, torch.tensor([0, 1, 2])).flip(-1)
...
def test_aiter_fp8_mha_v4_receives_unrotated_qk(monkeypatch):
    ...
    monkeypatch.setattr(attention_backend, "_use_aiter_mha_v4_fp8", lambda *_: True, raising=False)
    monkeypatch.setattr(attention_backend, "_aiter_native_fp8_format", lambda: 4, raising=False)
    monkeypatch.setattr(attention_backend, "_aiter_mha_v4", mha_v4, raising=False)
    monkeypatch.setattr(
        attention_backend,
        "FP8_HADAMARD_MATRIX",
        {torch.device("cpu"): _feature_reversal_matrix()},
    ...
    output, _ = attention_backend._aiter_fp8_attn_call(query, key, value, dropout_p=0.0, is_causal=False)
...
def test_aiter_fp8_legacy_attention_retains_qk_rotation(monkeypatch):
    ...
    monkeypatch.setattr(attention_backend, "_use_aiter_mha_v4_fp8", lambda *_: False, raising=False)
    monkeypatch.setattr(attention_backend, "_aiter_fp8_dense_attention", dense_attention, raising=False)
    monkeypatch.setattr(
        attention_backend,
        "FP8_HADAMARD_MATRIX",
    ...
    monkeypatch.setattr(
        attention_backend,
        "_get_fp8_hadamard_matrix",
        lambda head_dim, device: rotation,
        raising=False,
    )
    ...
    key = torch.randn_like(query)
    value = torch.randn_like(query)
    attention_backend._aiter_fp8_attn_call(query, key, value, dropout_p=0.0, is_causal=False)
...
def test_aiter_fp8_compiles_without_dtype_rewrite(monkeypatch):
    ...
    fake_aiter = SimpleNamespace(
        dtypes=SimpleNamespace(fp8=torch.float8_e4m3fn),
        per_tensor_quant=per_tensor_quant,
        flash_attn_fp8_pertensor_func=lambda query, key, value, **kwargs: query.to(torch.bfloat16),
    )
    monkeypatch.setattr(attention_backend, "aiter", fake_aiter, raising=False)
    monkeypatch.setattr(attention_backend, "AITER_FP8_HAS_DESCALE", True, raising=False)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        attention_backend,
        "_get_fp8_hadamard_matrix",
        lambda head_dim, device: torch.eye(head_dim, device=device),
        raising=False,
    )
