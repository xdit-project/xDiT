...
def _reference_jenga_mask(query, key, block_size, top_k, threshold):
    ...
    query_pool = query.reshape(batch, heads, blocks, block_size, head_dim).mean(dim=-2)
    key_pool = key.reshape(batch, heads, blocks, block_size, head_dim).mean(dim=-2)
    scores = torch.matmul(query_pool, key_pool.transpose(-1, -2))
    probabilities = torch.softmax(scores * head_dim**-0.5, dim=-1)
    sorted_probabilities, indices = probabilities.sort(dim=-1, descending=True)
    needed = (sorted_probabilities.cumsum(-1) <= threshold).sum(-1) + 1
    needed = needed.clamp(min=top_k, max=blocks)
    result = torch.zeros(batch, heads, blocks, blocks, dtype=torch.bool)
...
def test_block_mask_to_delta_lut():
    ...
    torch.testing.assert_close(lut[0, 0, 0, :3], torch.tensor([0, 2, 3], dtype=torch.int32))
    torch.testing.assert_close(lut[0, 0, 1, :2], torch.tensor([1, 2], dtype=torch.int32))


def test_jenga_scheduled_drop_rate_matches_reference():
    rates = [0.75, 0.85]
    assert jenga_scheduled_drop_rate(0, 50, rates) == 0.0
    assert math.isclose(jenga_scheduled_drop_rate(1, 50, rates), 0.75 * 10 / 49)
    assert math.isclose(jenga_scheduled_drop_rate(2, 50, rates), 0.75 * 20 / 49)
...
def test_vsa_probability_defaults_match_benchmark():
    assert xFuserArgs.__dataclass_fields__["vsa_prob_threshold"].default == 0.9
    assert RuntimeConfig.__dataclass_fields__["vsa_prob_threshold"].default == 0.9
    assert RuntimeConfig.__dataclass_fields__["vsa_collect_density"].default is False
    assert inspect.signature(build_jenga_block_mask).parameters["prob_threshold"].default == 0.9
...
def test_vsa_rejects_hybrid_attention_schedule():
    ...
    with pytest.raises(RuntimeError, match="hybrid attention schedule"):
        state._check_if_backend_compatible_with_current_configuration(AttentionBackendType.AITER_VSA)
...
def test_vsa_density_collection_is_opt_in():
    ...
    _, disabled_density = aiter_vsa_attention(query, key, value, collect_density=False, **kwargs)
    _, enabled_density = aiter_vsa_attention(query, key, value, collect_density=True, **kwargs)
...
def test_sliced_gilbert_permutation_matches_jenga_reference():
    forward, inverse = get_sliced_gilbert_perm((2, 3, 4), torch.device("cpu"))
