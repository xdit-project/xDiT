...
def test_h3_vsa_metadata_orders_full_video_tiles_first():
    ...
    assert metadata.first_partial_video_tile == (metadata.num_prefix_tiles + metadata.num_full_video_tiles)
...
def test_h3_vsa_bhsd_tile_round_trip(prefix_segments, video_shape):
    metadata = build_h3_vsa_metadata(prefix_segments=prefix_segments, video_shape=video_shape, device=CPU)
...
def test_h3_vsa_bhsd_tiling_matches_the_bshd_scatter():
    metadata = build_h3_vsa_metadata(prefix_segments=(3, 2), video_shape=(5, 5, 5), device=CPU)
...
def test_h3_vsa_packed_token_tile_maps_every_row_to_its_tile():
    ...
    counts = torch.bincount(metadata.packed_token_tile, minlength=metadata.num_tiles)
...
def test_h3_vsa_kv_blocks_select_the_reference_tile_set(prefix_segments, video_shape, sparsity):
    ...
    width = metadata.num_prefix_tiles + compute_h3_vsa_topk(sparsity, metadata.num_video_tiles)
    ...
    for counts, indices in (
        ...
        rebuilt.scatter_(-1, torch.where(valid, indices, num_tiles).long(), valid)
    ...
    for counts, indices, expect_partial in (
        ...
        assert torch.equal(padded[valid], torch.full_like(padded[valid], expect_partial))
    assert torch.equal(
...
def _reference_vsa_h3_attention(query, key, value, metadata, sparsity):
    ...
    scores = torch.matmul(pooled_query, pooled_key.transpose(-2, -1)) * (query.shape[-1] ** -0.5)
    block_map = build_h3_vsa_block_mask(scores, metadata.num_prefix_tiles, metadata.num_video_tiles, sparsity)
    token_mask = block_map.repeat_interleave(metadata.tile_elements, dim=-1).repeat_interleave(
        metadata.tile_elements, dim=-2
    )
    token_mask = token_mask & metadata.tiled_slot_valid.view(1, 1, 1, -1)
    sparse = torch.nn.functional.scaled_dot_product_attention(query, key, value, attn_mask=token_mask)
    compressed = torch.matmul(torch.softmax(scores, dim=-1), pooled_value)
    return sparse, compressed


@pytest.mark.accelerator
@pytest.mark.skipif(not torch.cuda.is_available(), reason="FlexAttention needs a GPU")
def test_flex_h3_vsa_attention_matches_the_dense_reference():
    torch.manual_seed(0)
    device = torch.device("cuda")
    metadata = build_h3_vsa_metadata(prefix_segments=(65, 3), video_shape=(5, 6, 7), device=device)
    shape = (1, 4, metadata.padded_seq_length, 64)
    query, key, value = (torch.randn(shape, device=device, dtype=torch.bfloat16) for _ in range(3))
    for tensor in (query, key, value):
        tensor[:, :, metadata.pad_slot_index] = 0

    sparse, compressed = flex_h3_vsa_attention(query, key, value, metadata)
    sparse_ref, compressed_ref = _reference_vsa_h3_attention(query, key, value, metadata, FASTH3_VSA_SPARSITY)
    ...
    torch.testing.assert_close(compressed.float(), compressed_ref.float(), rtol=2e-2, atol=2e-2)
...
def test_triton_vsa_h3_backend_falls_back_to_flex(monkeypatch):
    ...
    monkeypatch.setattr(vsa_h3_attention, "h3_vsa_triton_is_usable", lambda device: False)
    monkeypatch.setattr(attention_backend, "_warned_vsa_h3_triton_missing", False)
    fell_back, _ = attention_backend._triton_vsa_h3_attn_call(query, key, value, 0.0, False, kwargs)
    flex, _ = attention_backend._flex_vsa_h3_attn_call(query, key, value, 0.0, False, kwargs)
...
def test_h3_vsa_attention_backends_agree_with_the_dense_reference(use_triton):
    ...
    expected = untile_h3_vsa_bhsd(sparse_ref, metadata) + (
        compressed_ref.to(gate.dtype).index_select(2, metadata.packed_token_tile) * gate
    )
...
def test_h3_vsa_kv_list_matches_the_block_lists(prefix_segments, video_shape, sparsity):
    ...
    both = torch.cat(
        (
            ...
            torch.where(ranks < full_kv_num_blocks.unsqueeze(-1), full_kv_indices, -1),
        ),
        dim=-1,
    )
    from_blocks = both.sort(dim=-1, descending=True).values[..., :width]
    assert torch.equal(kv_list[..., :width].sort(dim=-1, descending=True).values, from_blocks)
...
def test_h3_vsa_triton_pooling_selects_the_same_tiles_as_torch():
    ...
    device = torch.device("cuda")
    metadata = build_h3_vsa_metadata(prefix_segments=(11, 90), video_shape=(9, 8, 11), device=device)
    for seed in range(8):
        ...
        reference = [pool_h3_vsa_tiles(tile_h3_vsa_bhsd(tensor, metadata), metadata) for tensor in packed]
        gathered = [vsa_h3_triton.triton_pool_h3_vsa_tiles(tensor, metadata) for tensor in packed]
...
def test_h3_vsa_tiled_to_packed_row_marks_padding_out_of_range():
    ...
    assert rows.shape == (metadata.padded_seq_length + metadata.tile_elements,)
    ...
    assert torch.equal(real.sort().values, torch.arange(metadata.total_seq_length, dtype=torch.int32))
    assert torch.equal(
