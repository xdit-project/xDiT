...
Run with:
    python -m pytest tests/unit/core/utils/test_checkpoint_io.py -v
"""
...
def sharded_checkpoint(tmp_path):
    ...
    (subfolder / "diffusion_pytorch_model.safetensors.index.json").write_text(json.dumps(index))
    return tmp_path
...
def test_resolve_repo_file_finds_local_file(single_file_checkpoint):
    resolved = resolve_repo_file(str(single_file_checkpoint), "transformer/diffusion_pytorch_model.safetensors")
    assert resolved == str(single_file_checkpoint / "transformer" / "diffusion_pytorch_model.safetensors")
...
def test_weight_map_from_local_single_file(single_file_checkpoint):
    ...
    expected_path = str(single_file_checkpoint / "transformer" / "diffusion_pytorch_model.safetensors")
...
def test_weight_map_from_local_shards(sharded_checkpoint):
    ...
    assert weight_map == {
        "blocks.0.weight": str(subfolder / "diffusion_pytorch_model-00001-of-00002.safetensors"),
        "blocks.1.weight": str(subfolder / "diffusion_pytorch_model-00002-of-00002.safetensors"),
    }
...
def test_shard_paths_from_local_single_file(single_file_checkpoint):
    ...
    paths = component_shard_paths(str(single_file_checkpoint), "transformer", "diffusion_pytorch_model")
    assert paths == {str(single_file_checkpoint / "transformer" / "diffusion_pytorch_model.safetensors")}


def test_shard_paths_from_local_shards(sharded_checkpoint):
    paths = component_shard_paths(str(sharded_checkpoint), "transformer", "diffusion_pytorch_model")
    subfolder = sharded_checkpoint / "transformer"
    assert paths == {
        str(subfolder / "diffusion_pytorch_model-00001-of-00002.safetensors"),
        str(subfolder / "diffusion_pytorch_model-00002-of-00002.safetensors"),
