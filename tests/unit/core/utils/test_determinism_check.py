...
def test_determinism_check_counts_every_failure(monkeypatch):
    outputs = [DiffusionOutput(videos=[np.array([value], dtype=np.float32)]) for value in (1.0, 2.0, 3.0, 4.0)]
    model = _make_runner(
        monkeypatch,
        num_iterations=4,
        determinism_check=2,
...
def test_report_ranks_option_normalizes_to_frozenset(add_args, value, expected, monkeypatch):
    monkeypatch.setenv("WORLD_SIZE", "4")
    parser = add_args(FlexibleArgumentParser(description="xDiT"))
...
def test_report_ranks_underscore_option_is_accepted(monkeypatch):
    ...
    assert xFuserArgs.from_cli_args(parsed).determinism_check_report_ranks == frozenset({0, 2})
...
def test_report_ranks_help_warns_about_large_output_files():
    parser = xFuserArgs.add_runner_args(FlexibleArgumentParser(description="xDiT"))
    action = next(action for action in parser._actions if action.dest == "determinism_check_report_ranks")
