...
Run with:
    python -m pytest tests/unit/model_executor/layers/test_fp8_linear_sentinel.py -v
"""
...
def make_layer_with_meta_sentinel(in_features=8, out_features=4):
    ...
    with torch.device("meta"):
        layer = xFuserFP8BlockScaleLinear(in_features, out_features, bias=False, dtype=torch.bfloat16, preshuffle=False)
        layer.weight_fp8 = torch.nn.Parameter(
...
def test_absorb_moves_loader_stored_fp8_out_of_weight():
    ...
    layer = xFuserFP8BlockScaleLinear(8, 4, bias=False, device="cpu", dtype=torch.bfloat16, preshuffle=False)
