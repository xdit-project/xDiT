...
CONTRACTS_PATH = ROOT / "xfuser/model_executor/models/runner_models/loading/contracts.py"
...
def test_declaration_defaults_are_explicitly_unsupported(contracts):
    ...
    assert declaration.materialization_modes == frozenset({contracts.MaterializationMode.EAGER})
    assert declaration.quantization_backends == frozenset({contracts.QuantizationBackend.NONE})
...
def test_declared_meta_mode_requires_every_transformer_in_strategy(contracts):
    declaration = contracts.LoadDeclaration.meta("transformer", "transformer_2", replicated=True)
...
def test_contract_selection_accepts_only_declared_backend_and_mode(contracts):
    ...
    assert selected.materialization_mode is contracts.MaterializationMode.REPLICATED_META
...
def test_int8_still_conflicts_with_hybrid_quantization(contracts):
    ...
    with pytest.raises(
        ...
        contracts.select_runtime_quantization(config, aiter_fp8_active=True, cuda_active=False)
...
def test_fsdp_and_replicated_meta_support_are_derived_separately(contracts):
    ...
    assert contracts.MaterializationMode.FSDP_META not in (only_replicated.materialization_modes)
...
def test_effective_replicated_mode_applies_runtime_exclusions(contracts, world_size, degrees, expected):
    config = type(
        "Config",
        (),
        {
    ...
    mode = contracts.select_effective_materialization_mode(config, world_size=world_size)
...
def test_a_replicated_request_that_would_be_dropped_is_refused(contracts, degrees, world_size, expected_in_reason):
    ...
    with pytest.raises(contracts.UnsupportedLoadContract) as refusal:
        contracts.assert_requested_materialization_is_honoured(config, world_size=world_size)
...
def test_group_offload_with_aiter_fp4_is_refused(contracts, low_cpu_mem, expected_in_reason):
    """Both legs of the offload fail below Python, one by abort, so neither can be caught."""
    config = _offload_config(enable_group_cpu_offload=True, group_offload_low_cpu_mem=low_cpu_mem)
...
def test_an_offload_this_has_no_measurement_for_is_left_alone(contracts, format_name, backend_name, offload):
...
def test_offload_that_reaches_inside_a_sharded_parameter_is_refused(contracts, flag, expected_in_reason):
...
def test_runtime_quantization_selection(contracts, flags, aiter_fp8, cuda, expected):
    ...
    requested, backend = contracts.select_runtime_quantization(config, aiter_fp8_active=aiter_fp8, cuda_active=cuda)
