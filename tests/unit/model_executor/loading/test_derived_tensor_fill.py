...
def test_a_fused_weight_becomes_one_projection_at_a_time():
    ...
    filler = _filler(manifest, _Shard({"qkv.weight": weight, "qkv.weight_scale": scale}))
...
def test_a_quantized_weight_is_read_with_the_scale_stored_beside_it():
    ...
    filler = _filler(manifest, _Shard({"linear.weight": weight, "linear.weight_scale": scale}))
...
def test_a_denoiser_is_recognised_by_the_declaration_not_by_its_name():
    ...
    loader.model = SimpleNamespace(
        load_declaration=SimpleNamespace(all_meta_transformers=("transformer", "unconditional_transformer"))
    )
