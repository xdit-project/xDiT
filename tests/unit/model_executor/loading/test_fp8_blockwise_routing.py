...
def runtime(model):
    ...
    return SimpleNamespace(
        ...
        replicated_broadcast_load=getattr(model, "_replicated_broadcast_load", lambda: False),
...
def test_wan_fp8_only_second_transformer_uses_blockwise_backend(monkeypatch):
    ...
    quantize = shard.build_block_quantize_fn(runtime(model), "transformer_2", ["blocks"], local_rank=3)
    block = object()
    quantize(block, 0)
...
def test_blockwise_fp8_only_target_honors_include_suffixes():
    ...
    quantize = shard.build_block_quantize_fn(runtime(model), "transformer_2", ["blocks"], local_rank=0)
    quantize(object(), 0)
...
def test_fp4_target_keeps_precision_overrides_in_fp4_owner(monkeypatch):
    ...
    quantize = shard.build_block_quantize_fn(runtime(model), "transformer", ["blocks"], local_rank=1)
    block = object()
    quantize(block, 3)
...
def test_blockwise_fp4_and_int8_route_through_format_adapter(
    ...
    quantize = shard.build_block_quantize_fn(runtime(model), "transformer", ["blocks"], local_rank=2)
    block = object()
    quantize(block, 2)
...
def test_descendant_target_quantizes_only_block_zero_subpath(format_name):
    adapter = FilterRecordingFp8Adapter() if format_name == "fp8" else RecordingFormatAdapter()
    ...
    quantize = shard.build_block_quantize_fn(runtime(model), "transformer", ["blocks"], local_rank=0)
...
def test_quantization_backends_preflight_resolves_required_adapters(monkeypatch, uses_blockwise, expected):
    from xfuser.model_executor.models.runner_models.loading.backend_selection import (
        QuantizationBackends,
    )
...
def test_fp8_fsdp_preflight_uses_boundary_safe_path_containment(
    ...
    assert backends(model).places_torchao_tensor_subclass_under_fsdp2(adapter) is expected
...
def test_eager_fp4_routes_fp8_only_module_by_hardware(
    ...
    fp4_calls = []
    fp8_calls = []
    format_adapter = SimpleNamespace(convert_module=lambda module, **kwargs: fp4_calls.append((module, kwargs)))
    ...
    model = SimpleNamespace(
        ...
        quantization_ledger=QuantizationLedger(descriptor_components={"transformer"}),
    )
