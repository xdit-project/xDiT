...
Run with:
    python -m pytest tests/unit/model_executor/loading/test_fp8_plan.py -v
"""
...
def test_model_loader_materialization_logs_fp4_overrides_and_places(monkeypatch):
    ...
    assert messages == [
        "The following layers will be quantized to FP8, to maintain output quality: ('blocks.0',) (prefix match)",
        "The following layers will be quantized to FP8, to maintain output quality: ('.proj',) (suffix match)",
    ]
...
def test_model_loader_materialization_uses_current_shard_degree(monkeypatch):
    ...
    calls = []
    model = SimpleNamespace(config=SimpleNamespace(use_fp4_gemms=False, fully_shard_degree=2))
    loader = SimpleNamespace(
        model=model,
        quantization_plan=SimpleNamespace(log_gemm_plan=lambda: None),
    )
    monkeypatch.setattr(shard, "shard_pipeline_components", lambda value: calls.append(("shard", value)))
    monkeypatch.setattr(
        placement,
...
def test_prefix_match_does_not_leak_across_sibling_components(monkeypatch):
    ...
    plan = make_plan(monkeypatch, transformer_targets=["transformer.blocks", "transformer_2.blocks"])
...
def test_no_runner_hides_a_text_encoder_in_the_always_on_list():
    ...
    for cls in dict.fromkeys(MODEL_REGISTRY.values()):
        stray = [
            entry for entry in (cls.settings.fp8_gemm_module_list or []) if "transformer" not in entry.partition(".")[0]
        ]
        if stray:
