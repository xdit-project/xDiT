...
def test_tensor_layout_contract_captures_specs_ties_and_persistence(runtime):
    ...
    module.register_buffer("cache", torch.ones(4, dtype=torch.float64), persistent=False)
...
def test_layout_contract_rejects_tensor_metadata_before_data_collective(runtime, field, local):
...
def test_disk_broadcast_reconciles_peer_shape_and_dtype_before_data(runtime):
    torch = runtime.torch
    module = torch.nn.Module()
    module.register_parameter("weight", torch.nn.Parameter(torch.empty(2, dtype=torch.float32)))
    ...
    runtime.meta._collective_reconcile_tensor_specs(module, ["weight"], Group(), device="cpu")
...
def test_replicated_te_reconciles_specs_before_exact_layout_validation(runtime):
    ...
    component.register_parameter("weight", torch.nn.Parameter(torch.empty(2, dtype=torch.float16)))
    source_contract = (
        (
            "parameter",
            "weight",
...
def test_source_failure_status_makes_every_rank_raise(runtime, rank):
    ...
    assert calls == (["operation", "broadcast_object_list"] if rank == 0 else ["broadcast_object_list"])
...
def test_collective_build_gate_reports_peer_failure_to_successful_rank(runtime):
    ...
    with pytest.raises(RuntimeError, match="meta transformer.*rank 1.*ValueError"):
        runtime.meta._collective_build_call(Group(), lambda: "built", context="meta transformer")
...
def test_collective_build_gate_is_noop_for_single_rank(runtime):
    ...
    assert runtime.meta._collective_build_call(group, lambda: "built", context="meta transformer") == "built"
    assert not calls
...
def test_block_fill_requires_and_broadcasts_only_persistent_buffers(runtime):
    ...
    fills = []
    filler._fill = lambda module, local_name, key, required: fills.append((local_name, key, required))
    broadcasts = []
...
def test_local_block_fill_uses_no_collective_transport(runtime):
    ...
    fills = []
    filler._fill = lambda module, local_name, key, required: fills.append((local_name, key))
...
def test_block_read_failure_is_collective_before_tensor_broadcast(runtime, rank):
    ...
    filler._fill = lambda *args, **kwargs: (_ for _ in ()).throw(OSError("safetensors read failed"))

    with pytest.raises(RuntimeError, match="loading transformer checkpoint tensors.*OSError"):
        filler.fill_block(block, 1)
...
def _prefetch_filler(runtime, monkeypatch, warmed, order, unread=None):
    ...
    filler._unread_by_shard = {p: {k for k, q in filler.weight_map.items() if q == p} for p in order}
...
def test_a_retired_shard_is_not_streamed_back_in(runtime, monkeypatch):
    ...
    """
    warmed = []
    filler = _prefetch_filler(runtime, monkeypatch, warmed, ["a", "b"], unread={"a": {"k0"}, "b": set()})
...
def test_shard_component_restores_nonpersistent_block_buffers_before_disk_callback(runtime, monkeypatch):
    torch = runtime.torch
    ...
    runtime.sharding.shard_component(
        ...
        load_block_fn=lambda block, index: seen.append(block.runtime_cache.detach().clone()),
    )
...
def test_replicated_transformer_restores_nonpersistent_buffers_before_fill(runtime, monkeypatch):
    torch = runtime.torch
...
def test_local_transformer_fills_and_quantizes_each_block_before_tail(
    ...
    torch = runtime.torch
    component = torch.nn.Module()
    component.blocks = torch.nn.ModuleList([torch.nn.Linear(1, 1), torch.nn.Linear(1, 1)])
    loader = object.__new__(runtime.meta.ModelLoader)
    loader.model = SimpleNamespace()
    events = []
...
@pytest.mark.parametrize("rank", [0, 1])
def test_replicated_quantize_failure_stops_all_ranks_before_next_block(runtime, monkeypatch, rank):
    torch = runtime.torch
    component = torch.nn.Module()
    component.blocks = torch.nn.ModuleList([torch.nn.Linear(1, 1), torch.nn.Linear(1, 1)])
    fills = []
    ...
    class Group:
        ...
        @staticmethod
        def broadcast_object_list(box, src=0):
            box[0] = ("ValueError", "rank-local quantization failed") if src == 1 else None
...
@pytest.mark.parametrize("rank", [0, 1])
def test_text_encoder_source_state_failure_is_collective_before_scatter(runtime, monkeypatch, rank):
    torch = runtime.torch
    object_broadcasts = []
    scatters = []
    releases = []
    ...
    loader = object.__new__(runtime.meta.ModelLoader)
    loader.model = SimpleNamespace(settings=SimpleNamespace(fsdp_strategy={"text_encoder": {"wrap_attrs": []}}))
    ...
    with pytest.raises(RuntimeError, match="loading text_encoder source.*OSError"):
        loader._broadcast_load_component(torch.nn.Linear(1, 1), "text_encoder", offload=False)
