...
Run with:
    python -m pytest tests/unit/model_executor/loading/test_replicated_load.py -v
"""
...
def loader_for(runner):
    ...
    """
    inner = getattr(runner, "loader", getattr(runner, "_loader", SimpleNamespace()))
    ...
    loader = SimpleNamespace(
        ...
        checkpoint_request=lambda subfolder=None, **kwargs: runner._checkpoint_request(subfolder, **kwargs),
        ...
        build_meta_transformer=getattr(inner, "build_meta_transformer", missing_route("build_meta_transformer")),
        build_meta_component=getattr(inner, "build_meta_component", missing_route("build_meta_component")),
        plan_eager_blockwise_fallback=getattr(inner, "plan_eager_blockwise_fallback", lambda *args, **kwargs: None),
        mark_local_blockwise=getattr(inner, "mark_local_blockwise", lambda component: None),
...
def test_fsdp_meta_load_needs_both_the_flag_and_sharding(monkeypatch):
    ...
    assert make_loader(monkeypatch, memory_efficient_sharding=True, fully_shard_degree=2).fsdp_meta_load()
...
def test_the_meta_transformer_keeps_the_wrappers_fp32_modules(monkeypatch):
    ...
    dtypes = dict((name, param.dtype) for name, param in built.named_parameters())
...
def test_custom_mapped_source_can_build_meta_only_for_local_fill(monkeypatch):
    ...
    source = CheckpointManifest(weight_map={"blocks.0.weight": "/weights/distilled.safetensors"})
...
def test_dual_meta_transformers_keep_distinct_checkpoint_requests(monkeypatch):
    loader = make_loader(monkeypatch)
    loader.load_declaration = LoadDeclaration.meta("transformer", "transformer_2", replicated=True)
    loader.model.settings.fsdp_strategy["transformer_2"] = {"wrap_attrs": ["blocks"]}
    first_request = CheckpointRequest("org/repo", subfolder="transformer", revision="main")
    second_request = CheckpointRequest("org/repo", subfolder="transformer_2", revision="refs/pr/9")
...
def test_runner_load_declarations_match_model_quantization_capabilities():
    ...
    assert not mismatched, "load_declaration quantization contracts disagree with ModelCapabilities: " + ", ".join(
        sorted(mismatched)
    )
...
def test_runner_fsdp_meta_support_matches_capabilities_and_strategy():
    ...
    for cls in dict.fromkeys(MODEL_REGISTRY.values()):
        ...
        expected = (
            tuple(name for name in candidates if cls.settings.fsdp_strategy.get(name, {}).get("wrap_attrs"))
            if cls.capabilities.fully_shard_degree
            else ()
        )
        if declaration.fsdp_meta_transformers != expected:
            mismatched.append(f"{cls.__name__}: expected {expected}, declared {declaration.fsdp_meta_transformers}")
...
def test_build_transformer_routes_torchao_fp8_to_native_diffusers_config(
    ...
    monkeypatch.setattr(transformer_load, "build_transformer_structure", structure_factory)
    monkeypatch.setattr(transformer_load, "native_quantization_device_map", lambda *args: {"": 0})
    runner = SimpleNamespace(
        _memory_efficient_fsdp_load=lambda: False,
        _replicated_broadcast_load=lambda: False,
...
def test_build_transformer_logs_explicit_torchao_post_load_fallback(monkeypatch):
    ...
    assert any(
        "backend=torchao" in message and "materialization=post_load" in message and "fallback=" in message
        for message in logs
    )
...
def test_eager_post_load_fallback_builds_meta_for_local_block_fill(monkeypatch):
    ...
    loader = SimpleNamespace(
        plan_eager_blockwise_fallback=lambda prepared, targets, wrap_attrs: (SimpleNamespace(enabled=True)),
...
def test_build_transformer_records_only_streamed_nvfp4_leaves(monkeypatch):
    ...
    monkeypatch.setattr(transformer_load, "build_transformer_structure", lambda *args, **kwargs: structure)
    monkeypatch.setattr(transformer_load, "native_quantization_device_map", lambda *args: {"": 0})
    runner = SimpleNamespace(
        _memory_efficient_fsdp_load=lambda: False,
        _replicated_broadcast_load=lambda: False,
...
def test_eager_te_adapter_maps_multiple_components_and_logs_each(monkeypatch):
    ...
    runner = SimpleNamespace(
        ...
        _loader=SimpleNamespace(build_meta_component=lambda name, fp8=False: (name, fp8)),
    )
    prepared = {
    ...
    monkeypatch.setattr(
        ...
        lambda self, mapping, existing=None: (setattr(sentinel, "quant_mapping", dict(mapping)) or sentinel),
    )
...
def test_hybrid_meta_te_uses_blockwise_fp8_backend(monkeypatch):
    ...
    runner = SimpleNamespace(
        load_contract=SimpleNamespace(requested_format=SimpleNamespace(value="fp8_fp4")),
        fp8_backend=None,
        blockwise_fp8_backend=sentinel,
        _replicated_broadcast_load=lambda: False,
        _memory_efficient_fsdp_load=lambda: True,
        fp8=SimpleNamespace(targets_for=lambda name: ["encoder.block"]),
        settings=SimpleNamespace(fp8_text_encoder_module_list=["text_encoder.encoder.block"]),
...
def test_a_blockwise_filled_text_encoder_needs_no_post_load_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "xfuser.model_executor.models.runner_models.loading.fp8_backends.prepare_text_encoder_fp8_load",
        lambda *a, **k: pytest.fail("a blockwise-filled encoder was routed to the whole-encoder rank0 load"),
    )
    monkeypatch.setattr(text_encoder_plan, "log", lambda message: None)
