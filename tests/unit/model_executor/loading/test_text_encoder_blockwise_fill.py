...
def make_loader(
    ...
    *,
    ...
    model = SimpleNamespace(
        ...
        load_declaration=SimpleNamespace(meta_text_encoders=tuple(meta_text_encoders)),
        quantization_ledger=QuantizationLedger(fp8_streaming_targets={"text_encoder.model.layers"}),
    )
...
def install(
    ...
    *,
    ...
    monkeypatch.setattr(meta_load, "get_world_group", lambda: world or SingleRankWorld())
    if world is not None and world.world_size > 1:
        real_tensor = meta_load.torch.tensor

        def cpu_tensor(*args, **kwargs):
            kwargs.pop("device", None)
            return real_tensor(*args, **kwargs)

        monkeypatch.setattr(meta_load.torch, "tensor", cpu_tensor)
    ...
    monkeypatch.setattr(
        "xfuser.model_executor.models.runner_models.loading.text_encoder_adapter.resolve_transformers_manifest",
        resolve,
    )
    monkeypatch.setattr(
        meta_load.ModelLoader,
...
def test_a_mapped_encoder_is_left_bf16_so_the_fill_matches_the_checkpoint(monkeypatch, meta_load):
...
def test_a_refused_encoder_is_swapped_to_fp8_for_the_broadcast_fallback(monkeypatch, meta_load):
    """The fallback broadcasts rank0's fp8 tensors, so the peer layout has to be fp8 to match."""
    loader = make_loader(meta_load)
    encoder = install(monkeypatch, meta_load, manifest=None, refusal="keys need a fused split")
...
def test_an_encoder_with_no_declared_blocks_is_refused_without_reading_the_checkpoint(monkeypatch, meta_load):
...
def test_a_rank_that_could_map_it_says_why_it_is_falling_back_anyway(monkeypatch, meta_load):
    loader = make_loader(meta_load)
    install(
        monkeypatch,
        meta_load,
...
def test_the_route_is_decided_once_so_the_plan_and_the_load_cannot_disagree(monkeypatch, meta_load):
...
def test_an_encoder_that_cannot_be_rebuilt_leaves_the_caller_its_normal_load(monkeypatch, meta_load):
    loader = make_loader(meta_load)
