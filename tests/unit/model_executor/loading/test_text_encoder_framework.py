...
ADAPTER_PATH = ROOT / "xfuser/model_executor/models/runner_models/loading/text_encoder_adapter.py"
BACKENDS_PATH = ROOT / "xfuser/model_executor/models/runner_models/loading/fp8_backends.py"
...
def _backend(modules, name, **capabilities):
    ...
    return b.select_fp8_backend(contract, capabilities=b.Fp8BackendCapabilities(**defaults))
...
def test_torchao_pipeline_mapping_uses_exact_exclusions_for_multiple_encoders(
    ...
    framework = a.TextEncoderFrameworkAdapter(
        ...
        torchao_config_factory=lambda exclusions: calls.append(exclusions) or ("torchao", tuple(exclusions)),
    )
...
def test_combined_mapping_rejects_component_overwrite(modules):
    a = modules.adapter
    framework = a.TextEncoderFrameworkAdapter(pipeline_config_factory=lambda mapping: mapping)
...
def test_installed_torchao_te_pipeline_uses_transformers_config(modules):
    ...
    assert pipeline_config._resolve_quant_config(is_diffusers=False, module_name="text_encoder") is te_config
...
def test_installed_pipeline_accepts_combined_transformer_and_te_mapping(
    ...
    existing = framework.pipeline_quantization_config({"transformer": transformer_config})
    te_config = framework.component_quantization_config(
        backend="torchao",
        targets=("encoder.block",),
        exclusions=("shared",),
    ...
    assert combined._resolve_quant_config(is_diffusers=True, module_name="transformer") is transformer_config
    assert combined._resolve_quant_config(is_diffusers=False, module_name="text_encoder") is te_config
