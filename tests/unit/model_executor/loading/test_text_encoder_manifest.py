...
class Encoder(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.layers = torch.nn.ModuleList([torch.nn.Linear(2, 2, bias=False) for _ in range(2)])
...
def resolve(component, discover, conversions=()):
    return resolve_transformers_manifest(component, REQUEST, discover=discover, conversions=list(conversions))
...
def test_the_prefix_is_decided_per_key_not_once_for_the_checkpoint():
    ...
    manifest, refusal = resolve(Encoder(), discovery("model.layers.0.weight", "layers.1.weight", "scale"))
...
def test_a_checkpoint_carrying_tensors_the_component_lacks_is_refused():
    ...
    manifest, refusal = resolve(Encoder(), discovery(*LIVE_KEYS, "layers.0.qkv_proj.weight"))
...
def test_two_checkpoint_keys_claiming_one_tensor_are_refused():
    manifest, refusal = resolve(Encoder(), discovery(*LIVE_KEYS, "model.layers.0.weight"))
...
def test_a_non_renaming_conversion_is_refused(monkeypatch):
    ...
    monkeypatch.setattr(modeling_utils, "get_model_conversion_mapping", lambda component: [Fusing()])

    manifest, refusal = resolve_transformers_manifest(Encoder(), REQUEST, discover=discovery(*LIVE_KEYS))
...
def test_the_manifest_points_every_live_name_at_its_shard():
    manifest, refusal = resolve(Encoder(), discovery(*LIVE_KEYS, path="/shard-a.safetensors"))
...
def test_a_component_without_a_base_model_prefix_only_accepts_exact_keys(prefix):
    ...
    prefixed, prefixed_refusal = resolve(encoder, discovery(*(f"model.{k}" for k in LIVE_KEYS)))
