...
def _runner(**config):
    ...
    runner._vae_manager = vae_manager.VAEManager(runner.config, runner.capabilities, runner.settings)
    return runner
...
def test_vae_tile_settings_document_tiling_requirement(add_args):
    parser = add_args(FlexibleArgumentParser(description="xDiT"))
    tile_settings = [action for action in parser._actions if action.dest.startswith("vae_tile_")]
...
def test_zero_overlap_is_valid_for_an_inactive_strip_axis():
    ...
    vae_manager.validate_vae_config(config, runner.capabilities, runner.settings)
...
def test_enable_tiling_without_dimensions_keeps_native_window():
    ...
    with (
        ...
        mock.patch.object(vae_manager.vae_tiling, "tile_shape", return_value=(512, 512)),
        ...
        mock.patch.object(runner._vae_manager, "_check_tiles_against_parallel_vae"),
...
def test_worker_vae_wrapper_uses_runtime_device_group_with_public_api():
    ...
    with (
        mock.patch.object(base_pipeline, "get_vae_parallel_group", return_value=coordinator),
        mock.patch.object(base_pipeline, "parallelize_decoder") as parallelize_decoder,
    ):
...
def test_worker_vae_wrapper_accepts_dedicated_raw_process_group():
    ...
    with (
        mock.patch.object(base_pipeline, "get_vae_parallel_group", return_value=process_group),
        mock.patch.object(base_pipeline, "parallelize_decoder") as parallelize_decoder,
    ):
        converted = base_pipeline.xFuserVAEWrapper._convert_vae(object(), vae)
...
def test_parallel_setup_adapts_runtime_group_for_distvae():
    ...
    with (
        mock.patch.object(vae_manager, "get_vae_parallel_group", return_value=coordinator),
...
def test_tile_overlap_is_applied_through_distvae():
    ...
    tile_overlap_plan.assert_called_once_with(vae, 32, 64, sample_shape=(320, 1280))
...
def test_tile_overlap_tracks_each_run_sample_shape():
    ...
    with mock.patch.object(runner._vae_manager, "_apply_vae_tile_overlap") as apply:
        runner._vae_manager.prepare_run([vae], {"height": 320, "width": 1280})
        runner._vae_manager.prepare_run([vae], {"height": 320, "width": 1280})
        runner._vae_manager.prepare_run([vae], {"height": 640, "width": 960})
...
def test_tile_overlap_refuses_an_inexact_plan_without_mutating_the_vae():
    ...
    with (
        mock.patch.object(vae_manager.vae_tiling, "tile_shape", return_value=(512, 768)),
        mock.patch.object(
            vae_manager.vae_tiling,
            "tile_overlap_plan",
            return_value=None,
...
def test_rectangular_tile_shape_is_applied_exactly_through_distvae():
    ...
    with (
        mock.patch.object(vae_manager.vae_tiling, "tile_shape_plan", return_value=plan) as tile_shape_plan,
        mock.patch.object(vae_manager.vae_tiling, "apply_tile_plan") as apply,
    ):
...
def test_rectangular_tile_shape_refuses_a_non_exact_plan_actionably():
    ...
    with mock.patch.object(vae_manager.vae_tiling, "tile_shape_plan", return_value=None):
        with pytest.raises(ValueError) as error:
...
def test_single_gpu_rectangular_scalar_vae_installs_undispatched_tiled_decode():
    ...
    with (
        mock.patch.object(vae_manager.vae_tile_parallel, "context_of", return_value=None),
        mock.patch.object(vae_manager.vae_tiling, "tiled_decode_for", return_value=installed) as tiled_decode_for,
    ):
...
def test_native_keyed_vae_keeps_its_tiled_decode_when_distvae_returns_none():
    ...
    with (
        mock.patch.object(vae_manager.vae_tile_parallel, "context_of", return_value=None),
        mock.patch.object(vae_manager.vae_tiling, "tiled_decode_for", return_value=None) as tiled_decode_for,
    ):
        runner._vae_manager._install_vae_tiled_decode(vae)
...
def test_tiled_decode_install_uses_distvae_context_and_sharing():
    ...
    with (
        mock.patch.object(vae_manager.vae_tile_parallel, "context_of", return_value=context),
        mock.patch.object(
...
def test_parallel_vae_rank_hint_increases_only_the_sharded_tile_axis():
    ...
    with (
        mock.patch.object(vae_manager.vae_tile_parallel, "context_of", return_value=None),
        mock.patch.object(vae_manager, "get_vae_parallel_world_size", return_value=2),
        mock.patch.object(vae_manager, "latent_rows", side_effect=rows),
        mock.patch.object(vae_manager.vae_tiling, "tile_shape", return_value=(64, 64)),
        mock.patch.object(vae_manager.vae_tiling, "tile_shape_plan", side_effect=shape_plan) as tile_shape_plan,
    ):
        with pytest.raises(ValueError) as error:
            runner._vae_manager._check_tiles_against_parallel_vae(vae, (512, 512))
...
def test_parallel_vae_accepts_rectangular_tile_with_enough_latent_height():
    ...
    with (
        mock.patch.object(vae_manager.vae_tile_parallel, "context_of", return_value=None),
        mock.patch.object(vae_manager, "get_vae_parallel_world_size", return_value=16),
    ):
        runner._vae_manager._check_tiles_against_parallel_vae(vae, native_shape=(512, 512))
...
def test_decode_guard_reports_rectangular_window_and_flags():
    ...
    with mock.patch.object(vae_manager.vae_tiling, "is_tile_padding_error", return_value=True) as is_padding:
        runner._vae_manager._install_vae_decode_guard(vae, tile_shape=(320, 512))
...
def test_decode_guard_suggests_complete_tiles_for_incompatible_row_sharding():
    ...
    with mock.patch.object(vae_manager.vae_tiling, "supports_tile_parallel", return_value=True):
...
def test_decode_oom_hint_reports_rectangular_window():
    ...
    with mock.patch.object(vae_manager.vae_tiling, "tile_shape", return_value=(320, 512)):
