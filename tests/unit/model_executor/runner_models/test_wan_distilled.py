...
Run with:
    python -m pytest tests/unit/model_executor/runner_models/test_wan_distilled.py -v
"""
...
class TestDistilledWanScheduler:
    ...
    def test_num_inference_steps_is_ignored_by_schedule(self, scheduler):
        ...
        assert torch.allclose(ts_4, ts_20), "Schedule must be fixed regardless of num_inference_steps"
...
class TestDistilledTransformerLoading:
    def test_quantized_load_uses_mapped_local_blockwise_source(self):
        ...
        model.loader = SimpleNamespace(
            transformer_quantization_adapter=MagicMock(return_value=(object(), ("blocks",))),
        ...
        with (
            patch.object(wan, "resolve_mapped_checkpoint", return_value=manifest) as resolve,
            patch.object(wan, "_build_attention_kwargs", return_value=attention_kwargs),
        ):
            result = model._build_distilled_transformer("transformer", "/weights/high.safetensors")
