...
class TestDiffusersImportErrors(unittest.TestCase):
    ...
    def test_diffusers_origin_is_recognised(self):
        ...
        self.assertTrue(is_diffusers_import_error(self._error("diffusers.models.transformers.transformer_z_image")))
...
class TestDeclaredFloor(unittest.TestCase):
    ...
    def _with_requires(self, requires):
        return patch.object(importlib.metadata, "requires", lambda name: requires, create=False)
...
class TestReexportChain(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (root / "sub" / "gated.py").write_text("from diffusers import OnlyInNewDiffusers\nGated = 'gated'\n")
        exporter = (
        ...
        (root / "sub" / "__init__.py").write_text(exporter + "_optional('.gated', 'Gated')\n")
