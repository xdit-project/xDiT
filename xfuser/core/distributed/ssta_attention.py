...
def create_ssta_3d_mask(q, 
    ...
    if text_mask is not None:
        block_size = np.prod(tile_thw)
        seq_len = q.size(2)
        block_num = torch.tensor((seq_len / block_size), dtype=torch.int)
        text_mask_index = torch.ceil(text_mask.sum() / block_size).long()
        text_mask_index = text_mask_index.clamp(min=1)
...
def expand_block_mask(mask_coarse: np.ndarray, factor: int) -> np.ndarray:
    """
...
def ssta_3d_attention(all_q, 
    ...
    if text_len % block_size > 0:
        ...
        if pad_type == "zero":
            pad_text_q = torch.zeros_like(pad_text_q)
            pad_text_k = torch.zeros_like(pad_text_k)
            pad_text_v = torch.zeros_like(pad_text_v)
    ...
    if sparse_type == 'sta':
        ...
        o = flex_block_attn_func(q, k, v, block_mask, block_size, block_size)
