...
from xfuser.core.distributed import get_sp_group, get_runtime_state
...
class xFuserCogVideoXLongContextAttention(LongContextAttention):
    ring_impl_type_supported_kv_cache = ["basic"]
    ...
    def forward(
        ...
        *,
        ...
        if self.use_pack_qkv:
            ...
            # cogvideox go this way
            query_layer = SeqAllToAll4D.apply(
                self.ulysses_pg, query[:, get_runtime_state().pp_patches_token_start_end_idx_global[0][0]\
                    : get_runtime_state().pp_patches_token_start_end_idx_global[0][1], ...], self.scatter_idx, self.gather_idx
            )
            key_layer = SeqAllToAll4D.apply(
                self.ulysses_pg, key[:, get_runtime_state().pp_patches_token_start_end_idx_global[0][0]\
                    : get_runtime_state().pp_patches_token_start_end_idx_global[0][1], ...], self.scatter_idx, self.gather_idx
            )
            value_layer = SeqAllToAll4D.apply(
                self.ulysses_pg, value[:, get_runtime_state().pp_patches_token_start_end_idx_global[0][0]\
                    : get_runtime_state().pp_patches_token_start_end_idx_global[0][1], ...], self.scatter_idx, self.gather_idx
            )
        ...
        #     self.ulysses_pg, context_layer, self.gather_idx, self.scatter_idx
        # )
        # all gather the output along seq_len dimension
        output_list = get_sp_group().all_gather(context_layer, separate_tensors=True)
