...
from xfuser.compat import reraise_unless_diffusers
...
# The FLUX transformer symbols only exist on newer diffusers versions, so register
# what this install has. The guard keys off where the ImportError came from: a missing
# diffusers symbol reached through the xfuser wrapper still counts as a version
# mismatch, while a broken xfuser import surfaces as an error.
try:
