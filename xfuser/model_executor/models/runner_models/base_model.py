...
from xfuser.core.utils import vae_parallel, vae_setup, vae_tile_parallel, vae_tiling
...
class xFuserModel(abc.ABC):
    ...
    def _tiling_request(self) -> vae_setup.TilingRequest:
        """ What this run asks of its VAE decode, for the module that decides what that means """
        # The VAE group is only asked about once this run is actually splitting tiles across it,
        # since get_vae_parallel_world_size() needs the group to have been built.
        splits_tiles = bool(self.config.use_parallel_vae and self.capabilities.use_parallel_vae)
        return vae_setup.TilingRequest(
            model_name=self.settings.model_name,
            tile_size=self.config.vae_tile_size,
            tile_overlap=self.config.vae_tile_overlap,
            tiling=bool(self.config.enable_tiling),
            slicing=bool(self.config.enable_slicing),
            model_tiles=bool(self.capabilities.enable_tiling),
            splits_tiles=splits_tiles,
            ranks=get_vae_parallel_world_size() if splits_tiles else 1,
        )
    ...
    def _setup_parallel_vae(self) -> None:
        ...
        request = self._tiling_request()
        for vae in self._decoding_vaes():
            if vae_setup.tiles(vae, request) and vae_tiling.supports_tile_parallel(vae):
